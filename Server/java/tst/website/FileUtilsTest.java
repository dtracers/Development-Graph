...
public class FileUtilsTest {
	...
	public static void createFakeProjectAndServer() throws IOException {
		...
		serv = new TestServer("localHost", 9001); // Specific port for this test!
		serv.start();
	}

	@Test
	public void testReading() throws IOException, InterruptedException, URISyntaxException {
		String identifier = "testReading";
		String testUrl = serv.getTestUrl(identifier, "fileHandler/fileCreationAndReadingTest.html?noglobals");
	...
	@Test
	public void testWriting() throws IOException, InterruptedException, URISyntaxException {
		String identifier = "testWriting";
		String testUrl = serv.getTestUrl(identifier, "fileHandler/fileWritingTest.html?noglobals");
		java.awt.Desktop.getDesktop().browse(new URI(testUrl));
		//serv.pause(TIMEOUT_TIME, identifier);
		assertTrue(serv.testEnded(identifier));
		assertEquals(0, serv.getNumberOfFailedTest());

		// now we actaully confirm that the writing has taken place.
	}
