...
public class SimpleWebServer extends NanoHTTPD {
    ...
    public boolean isUriSafe(String uri) {
    	return !(uri.startsWith("src/main") || uri.endsWith("src/main") || uri.contains("../"));
    }
    ...
    private Response respond(Map<String, String> headers, IHTTPSession session, String uri) {
        ...
        File homeDir = getRootDirs().get(0);

        File f;
		try {
			f = translatePath(homeDir, uri);
		} catch (Exception e) {
			return getForbiddenResponse(e.getMessage());
		}
        canServeUri = canServeUri(f);
    ...
    private boolean canServeUri(File f) {
        boolean canServeUri;
